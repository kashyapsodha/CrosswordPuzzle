...
//  CrosswordPuzzle
//
//  Created by Kashyap Sodha on 3/13/19.
...
class ViewController: UIViewController {
    ...
    // To check when field editing performed
    ...
    // Start the game by initializing everything
    func startGame(){
        board.startGame()
    ...
    // Checking if inserted word is equal to the original answer's word
    func checkWord() {
    ...
    // Refreshing the board for next word
    func refreshBoard() {
        grid = board.grid
        for y in 0...8{
            for x in 0...8{
    ...
    // Updating the score label based on correct words
    func updateScoreLabel() {
        scoreLabel?.text = "\(score)"
    }
    
    // Displaying values for reducing time
    ...
    // Display thumbs-up on every correct word
    ...
    // Decreasing time value 1 minute to 0 seconds
    ...
    override func viewDidLoad() {
        ...
        // To make score label value to '0'
        updateScoreLabel()
